...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
   ...
   clusters model_fake(N, D, x, phi, alphah, hyper, Mproc, useSuperclusters, always_splittable);
   model_fake.initialize(cluster_params);
   ...
   stopwatch timer;
   timer.tic();
   EArr[0] = model_fake.joint_loglikelihood();
   K[0] = model_fake.getK();
   NK[0] = model_fake.getNK();
   double time = timer.toc();
   timeArr[0] = time;

   
   model_fake.write_output(plhs[0]);
}
